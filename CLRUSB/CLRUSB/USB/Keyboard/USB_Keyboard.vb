...
#End Region
        ...
        Sub New(ByRef _Log As CLR_PSE_PluginLog, ByVal Config As Config.ConfigDataKeyboard)
            MyBase.New(_Log)
        ...
        Protected Overrides Function handle_control(ByVal request As Integer, ByVal value As Integer, ByVal index As Integer, ByVal length As Integer, ByRef data() As Byte) As Integer
            ...
            Select Case (request)
                ...
                Case SET_REPORT 'LEDs
                    ...
                    If NumLockOn Then
                        log.ErrorWriteLine("NumPad ON")
                    Else
                        log.ErrorWriteLine("NumPad OFF")
                    End If
                    If CapsLockOn Then
                        log.ErrorWriteLine("CapsLock ON")
                    Else
                        log.ErrorWriteLine("CapsLock OFF")
                    End If
                    If ScrollLockOn Then
                        log.ErrorWriteLine("ScrollLock ON")
                    Else
                        log.ErrorWriteLine("ScrollLock OFF")
                    End If
                    If (data(0) And (LED_NUMPAD Or LED_CAPS Or LED_SCROLL)) = 0 AndAlso data(0) <> 0 Then
                        log.ErrorWriteLine("Unkown LED ON")
                        'GoTo fail
                    End If
                    ret = 0
                Case SET_IDLE
                ...
                Case Else
fail:
                    log.ErrorWriteLine("STALL with request=" & request & ", value=" & value & ", index=" & index & ", length = " & length)
                    ret = USB_RET_STALL
            End Select
            Return ret
        End Function
...
fail:
                    ret = USB_RET_STALL
                    log.ErrorWriteLine("STALL on data")
            End Select
            Return ret
        End Function
        ...
        Public Overrides Function open(_hWnd As IntPtr) As Integer
            ...
            End Select
            '
            log.LogWriteLine("Open Keyboard")
            CaptureKeyboard = True
            Return 0
        End Function
        Public Overrides Sub close()
            ...
            log.LogWriteLine("Close Keyboard")
        End Sub
        Private Sub kbHook_KeyDown(ByVal Key As Windows.Forms.Keys) Handles hostKeyboard.KeyDown
            SyncLock KeysPressed
            ...
            End SyncLock
        End Sub
        Private Sub kbHook_KeyUp(ByVal Key As Windows.Forms.Keys) Handles hostKeyboard.KeyUp
            SyncLock KeysPressed
                KeysPressed.Remove(Key)
            End SyncLock
        End Sub

        Public Overrides Sub Freeze(ByRef freezedata As CLR_FreezeData, index As Integer, save As Boolean)
