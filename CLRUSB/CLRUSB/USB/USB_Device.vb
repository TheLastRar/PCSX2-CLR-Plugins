...
Namespace USB
    MustInherit Class USB_Device
        ...
        Protected log As CLR_PSE_PluginLog

        Sub New(ByRef _Log As CLR_PSE_PluginLog)
            log = _Log
        End Sub
        ...
        Public Function handle_packet(ByVal pid As Integer, ByVal devaddr As Byte,
            ...
            Select Case pid
                ...
                Case Else
fail:
                    log.ErrorWriteLine("STALL with pid=" & pid & ", devAddr=" & devaddr & ", devEp=" & devep & ", len=" & len)
                    ret = USB_RET_STALL
            End Select
            Return ret
        End Function
        ...
        Public Overridable Sub Freeze(ByRef freezedata As CLR_FreezeData, index As Integer, save As Boolean)
