...
#End Region
        ...
        Public USBLog As CLR_PSE_PluginLog
        ...
        Sub New(ByVal base As UInteger, ByVal ports As Integer, ByRef _USBLog As CLR_PSE_PluginLog)
            USBLog = _USBLog
            ...
            If (usb_frame_time = 0) Then
                ...
                Else
                    usb_bit_time = 1&
                End If
                USBLog.LogWriteLine("usb-ohci: usb_bit_time=" & usb_bit_time & ", usb_frame_time=" & usb_frame_time)
            End If
            num_ports = ports
            For i = 0 To ports - 1
        ...
        Private Sub cpu_physical_memory_rw(addr As UInt32, buffer As Byte(), len As Integer, is_write As Boolean)
            ...
            Else
                ...
                If FULL_DEBUG Then
                    USBLog.LogWrite("USB:IOR: IOPmem read")
                    For i As Integer = 0 To len - 1
                        USBLog.LogWrite(":" & buffer(i))
                    Next
                    USBLog.LogWriteLine("")
                End If
            End If
        ...
        Private Sub intr_update()
            ...
            If (intr And OHCI_INTR_MIE) <> 0 AndAlso (bits <> 0) Then
                If ((ctl And OHCI_CTL_HCFS) = OHCI_USB_OPERATIONAL) Then
                    If FULL_DEBUG Then
                        USBLog.LogWriteLine("usb-ohci: USBirq")
                    End If
                    USBirq(1)
                End If
            End If
        ...
        Private Sub die()
            USBLog.ErrorWriteLine("ohci_die: DMA error")
        ...
        Sub Reset()
            ...
            For i = 0 To num_ports - 1
                ...
                End If
            Next
            USBLog.LogWriteLine("usb-ohci: Reset")
            '}
            clocks = 0
            eof_timer = 0
            sof_time = 0
...
#End Region
        ...
        Private Function service_iso_td(ByRef ed As ohci_ed, ByVal compleation As Boolean) As Boolean 'compleation never gets set
            ...
            If Not (read_iso_td(addr, iso_td)) Then
                USBLog.ErrorWriteLine("usb-ohci: ISO_TD read error at " & addr.ToString("X"))
                die()
                Return False
            End If
            ...
            If (relative_frame_number < 0) Then '//aka don't start transfer yet i think it means
                USBLog.LogWriteLine("usb-ohci: ISO_TD R=" & relative_frame_number & " < 0")
                Return True
            ElseIf (relative_frame_number > frame_count) Then
                '/* ISO TD expired - retire the TD to the Done Queue and continue with
                '   the next ISO TD of the same ED */
                USBLog.LogWriteLine("usb-ohci: ISO_TD R=" & relative_frame_number & " < FC=" & frame_count)
            ...
            Select Case (dir)
                ...
                Case OHCI_TD_DIR_SETUP
                    pid = USB_TOKEN_SETUP
                Case Else
                    USBLog.ErrorWriteLine("usb-ohci: Bad direction " & dir)
                    Return True
            End Select

            If (Not (iso_td.bp <> 0) OrElse Not (iso_td.be <> 0)) Then
                USBLog.LogWriteLine("usb-ohci: ISO_TD bp 0x" & iso_td.bp & " be 0x" & iso_td.be)
                Return True
            End If
            ...
            If (
                ...
                USBLog.LogWriteLine("usb-ohci: ISO_TD cc != not accessed 0x" _
            ...
            If ((relative_frame_number < frame_count) AndAlso (start_offset > next_offset)) Then
                USBLog.LogWriteLine("usb-ohci: ISO_TD start_offset=0x" & start_offset.ToString("X") & " > next_offset=0x" & next_offset.ToString("X"))
                Return True
            End If
            ...
            Else
                If (ret > Convert.ToInt32(len)) Then
                    USBLog.ErrorWriteLine("usb-ohci: DataOverrun " & ret & " > " & len)
                    iso_td.offset(relative_frame_number) _
                        = CUShort(OHCI_SET_BM32(iso_td.offset(relative_frame_number), OHCI_TD_PSW_CC_MASK, OHCI_TD_PSW_CC_SHIFT,
                                OHCI_CC_DATAOVERRUN))
                    ...
                    iso_td.offset(relative_frame_number) _
                        = CUShort(OHCI_SET_BM32(iso_td.offset(relative_frame_number), OHCI_TD_PSW_SIZE_MASK, OHCI_TD_PSW_SIZE_SHIFT,
                               CUInt(len)))
                ElseIf (ret >= 0) Then
                    USBLog.ErrorWriteLine("usb-ohci: DataUnderrun " & ret)
                ...
                Else
                    Select Case (ret)
                        ...
                        Case USB_RET_NAK, USB_RET_STALL
                            USBLog.LogWriteLine("usb-ohci: got NAK/STALL " & ret)
                            iso_td.offset(relative_frame_number) _
                                = CUShort(OHCI_SET_BM32(iso_td.offset(relative_frame_number), OHCI_TD_PSW_CC_MASK, OHCI_TD_PSW_CC_SHIFT,
                                    OHCI_CC_STALL))
                            ...
                            iso_td.offset(relative_frame_number) _
                                = CUShort(OHCI_SET_BM32(iso_td.offset(relative_frame_number), OHCI_TD_PSW_SIZE_MASK, OHCI_TD_PSW_SIZE_SHIFT,
                                    0))
                        Case Else
                            USBLog.ErrorWriteLine("usb-ohci: Bad device response " & ret)
                            iso_td.offset(relative_frame_number) _
                                = CUShort(OHCI_SET_BM32(iso_td.offset(relative_frame_number), OHCI_TD_PSW_CC_MASK, OHCI_TD_PSW_CC_SHIFT,
        ...
        Private Function service_td(ByRef ed As ohci_ed) As Boolean
            ...
            If Not (read_td(addr, td)) Then
                USBLog.ErrorWriteLine("usb-ohci: TD read error at " & addr.ToString("X"))
                die()
            ...
            Select Case (dir)
                ...
                Case OHCI_TD_DIR_SETUP
                    ...
                    pid = USB_TOKEN_SETUP
                Case Else
                    USBLog.LogWriteLine("usb-ohci: Bad direction")
            ...
            Else
                If (ret >= 0) Then
                    USBLog.ErrorWriteLine("usb-ohci: Underrun")
                ...
                Else
                    Select Case (ret)
                        ...
                        Case USB_RET_BABBLE
                            USBLog.LogWriteLine("usb-ohci: got BABBLE")
        ...
        Private Function service_ed_list(head As UInt32) As Boolean
            ...
            While cur <> 0
                If Not (read_ed(cur, ed)) Then
                    USBLog.ErrorWriteLine("usb-ohci: ED read error at " & cur.ToString("X"))
                    die()
                    Return False 'return 0
                End If
        ...
        Public Sub frame_boundary()
            ...
            If ((ctl And OHCI_CTL_CLE) <> 0 AndAlso (status And OHCI_STATUS_CLF) <> 0) Then
                If (ctrl_cur <> 0 AndAlso ctrl_cur <> ctrl_head) Then
                    USBLog.LogWriteLine("usb-ohci: head " & ctrl_head.ToString("X") & ", cur " & ctrl_cur)
                End If
        ...
        Private Function bus_start() As Integer
            eof_timer = 0
            USBLog.LogWriteLine("usb-ohci: USB Operational")
            sof()
            Return 1
            '}
        End Function
        ...
        Private Function port_set_if_connected(i As Integer, val As UInt32) As Boolean
            ...
            If Not ((rhport(i).ctrl And OHCI_PORT_CCS) <> 0) Then
                ...
                If (rhstatus And OHCI_RHS_DRWE) <> 0 Then
                    ...
                    USBLog.LogWriteLine("usb-ohci: Not Implemented: CSC is a wakeup event")
                End If
                Return False
            End If
        ...
        Public Sub set_frame_interval(val As UInt32) 'This is meant to be Uint16, but fragment sends a massive value
            ...
            If (val <> fi) Then
                USBLog.LogWriteLine("usb-ohci: FrameInterval = 0x" & fi.ToString("X") & " (" & fi & ")")
            End If
        ...
        Private Sub set_ctl(val As UInt32)
            ...
            Select Case (new_state)
                ...
                Case OHCI_USB_SUSPEND
                    bus_stop()
                    USBLog.LogWriteLine("usb-ohci: USB Suspended")
                Case OHCI_USB_RESUME
                    USBLog.LogWriteLine("usb-ohci: USB Resume")
                Case OHCI_USB_RESET
                    USBLog.LogWriteLine("usb-ohci: USB Reset")
            End Select
            intr_update()
        End Sub
        ...
        Private Sub set_hub_status(val As UInt32)
            ...
            If (val And OHCI_RHS_LPS) <> 0 Then
                ...
                For i = 0 To num_ports - 1
                    port_power(i, False)
                    USBLog.LogWriteLine("usb-ohci: powered down all ports")
                Next
            End If
            ...
            If (val And OHCI_RHS_LPSC) <> 0 Then
                ...
                For i = 0 To num_ports - 1
                    port_power(i, True)
                    USBLog.LogWriteLine("usb-ohci: powered up all ports")
                Next
            End If
        ...
        Private Sub port_set_status(portnum As Integer, val As UInt32)
            ...
            If (port_set_if_connected(portnum, val And OHCI_PORT_PSS)) Then
                USBLog.LogWriteLine("usb-ohci: port " & portnum & ": SUSPEND")
            End If

            If (port_set_if_connected(portnum, val And OHCI_PORT_PRS)) Then
                USBLog.LogWriteLine("usb-ohci: port " & portnum & ": RESET")
        ...
        Public Function mem_read(addr As UInt32) As UInt32
            ...
            If (addr And 3) <> 0 Then
                USBLog.ErrorWriteLine("Error: usb-ohci: Mis-aligned read")
                Return &HFFFFFFFFUI
            End If
            ...
            Select Case (addr >> 2)
                ...
                Case 20 '/* HcRhStatus */
                    Return rhstatus
                Case Else
                    USBLog.ErrorWriteLine("Error: ohci_write: Bad offset " & addr.ToString("X"))
                    Return &HFFFFFFFFUI
            End Select
            '}
        End Function
        ...
        Public Sub mem_write(addr As UInt32, val As UInt32)
            ...
            If (addr And 3) <> 0 Then
                USBLog.ErrorWriteLine("Error: usb-ohci: Mis-aligned write")
                Return
            End If
            ...
            Select Case (addr >> 2UI)
                ...
                Case 20 '/* HcRhStatus */
                    ...
                    set_hub_status(val)
                Case Else
                    USBLog.LogWriteLine("Error: ohci_write: Bad offset " & addr.ToString("X")) 'was not stderr in original
            End Select
            '}
        End Sub

        'wish i knew a better way to do this
        Public Sub Freeze(ByRef FreezeData As CLR_FreezeData, save As Boolean)
