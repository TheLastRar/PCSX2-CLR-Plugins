...
struct PluginLog
{
	...
	void Write(const char *fmt)
	{
		...
		//va_start(list, fmt);
		if (WriteToFile) fprintf(LogFile, fmt);
		if (WriteToConsole) fprintf(stdout, fmt);
		//va_end(list);
	}
...
ref struct PluginConf
{
	...
	{
		return Open(name, READ_FILE);
	}
	bool Open(std::string name, FileMode mode)
	{
		if (mode == READ_FILE)
		{
